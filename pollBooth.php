 ...
 * @param   [type]  $voteID     [$voteID description]
 * @param   [type]  $forwarder  [$forwarder description]
 *
...
function pollCollector($pollID, $voteID, $forwarder)
{
    if ($voteID) {
        ...
        $result = sql_query("SELECT timeStamp 
                             FROM " . sql_table('poll_desc') . " 
                             WHERE pollID='$pollID'");

        list($timeStamp) = sql_fetch_row($result);

        $cookieName = 'poll' . sql_table($timeStamp);
        ...
        if ($setCookies == "1") {
            ...
            $sql = "SELECT al_id 
                    FROM " . sql_table('appli_log') . " 
                    WHERE al_id='$al_id' 
                    AND al_subid='$pollID' 
                    AND (al_ip='$ip' " . $hostname . $user_req . ")";
        ...
        if ($voteValid == "1") {
            ...
            sql_query("UPDATE " . sql_table('poll_data') . " 
                       SET optionCount=optionCount+1 
                       WHERE (pollID='$pollID') 
                       AND (voteID='$voteID')");

            sql_query("UPDATE " . sql_table('poll_desc') . " 
                       SET voters=voters+1 
                       WHERE pollID='$pollID'");
        }
    }
...
function pollList()
{
    ...
    while ($object = sql_fetch_assoc($result)) {
        ...
        $result2 = sql_query("SELECT SUM(optionCount) AS SUM 
...
function pollResults(int $pollID): void
{
    ...
    $result = sql_query("SELECT SUM(optionCount) AS SUM 
                         FROM " . sql_table('poll_data') . " 
                         WHERE pollID='$pollID'");
    ...
    for ($i = 1; $i <= $maxOptions; $i++) {
        $result = sql_query("SELECT optionText, optionCount, voteID 
                             FROM " . sql_table('poll_data') . " 
                             WHERE (pollID='$pollID') 
                             AND (voteID='$i')");
...
/**
 * Construit le blocs sondages
 * code du mainfile avec autre présentation
 *
 * @param   [type]  $pollID     [$pollID description]
...
{
    echo '<p><strong><a href="pollBooth.php">' . translate("Anciens sondages") . '</a></strong></p>';
}
