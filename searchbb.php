...
$term = Hack::removeHack(stripslashes(htmlspecialchars(urldecode($term), ENT_QUOTES, cur_charset))); // electrobug
...
$rowQ1 = Q_Select("SELECT forum_name, forum_id 
                   FROM " . sql_table('forums'), 3600);
...
$query = "SELECT u.uid, f.forum_id, p.topic_id, p.post_id, u.uname, p.post_time, t.topic_title, f.forum_name, f.forum_type, f.forum_pass, f.arbre 
          FROM " . sql_table('posts') . " p, " . sql_table('users') . " u, " . sql_table('forums') . " f, " . sql_table('forumtopics') . " t";
...
if (isset($username) && $username != '') {
    $username = Hack::removeHack(stripslashes(htmlspecialchars(urldecode($username), ENT_QUOTES, cur_charset))); // electrobug
