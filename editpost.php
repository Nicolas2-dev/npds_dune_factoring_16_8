...
if ($submitS) {
    ...
    if ($userdata[0] == $row['poster_id']) {
        ...
        if ((User::user_is_moderator($userdata[0], $userdata[2], $forum_access) < 2)) {
            Error::code('0036');
        }
    }
    ...
    if (($forum_type != 6) and ($forum_type != 5)) {
        ...
        $message .= '<div class="text-body-secondary text-end small">
            <i class="fa fa-edit"></i>&nbsp;' . translate("Message édité par") . " : " . $userdata['uname'] . " / " . Date::post_convertdate(time() + ((int)$gmt * 3600)) . '
            </div>';
    } else {
        $message .= "\n\n" . translate("Message édité par") . " : " . $userdata['uname'] . " / " . Forum::post_convertdate(time() + ((int)$gmt * 3600));
    }
    ...
    if (!isset($delete)) {
        ...
        $sql = "UPDATE " . sql_table('posts') . " 
                SET post_text = '$message', image='$image_subject' 
                WHERE (post_id = '$post_id')";
        ...
        $sql = "UPDATE " . sql_table('forumtopics') . " 
                SET topic_title = '$subject', topic_time = '" . date("Y-m-d H:i:s", time() + ((int) $gmt * 3600)) . "', current_poster='" . $userdata['uid'] . "' 
        ...
        if (!$indice) {
            ...
            $sql = "DELETE FROM " . sql_table('posts') . " 
                    WHERE post_id='$post_id'";
            ...
            if (Forum::get_total_posts($forum, $row['topic_id'], "topic", $Mmod) == 0) {
                ...
                $sql = "DELETE FROM " . sql_table('forumtopics') . " 
                        WHERE topic_id = '" . $row['topic_id'] . "'";
                ...
                $sql = "DELETE FROM " . sql_table('forum_read') . " 
                        WHERE topicid = '" . $row['topic_id'] . "'";
                ...
                $sql = "UPDATE " . sql_table('forumtopics') . " 
                        SET topic_time = '$rowX[0]', current_poster='$rowX[1]' 
                        WHERE topic_id = '" . $row['topic_id'] . "'";
    ...
    $sql = "SELECT p.*, u.uname, u.uid, u.user_sig 
    ...
    if ($submitP) {
        ...
        $message = stripslashes(Str::make_clickable($message));
