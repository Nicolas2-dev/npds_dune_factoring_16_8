            ...
            if ($r) {
                if (!list($rid) = sql_fetch_row($r)) {
                    $r = sql_query("INSERT 
                                    ...
                                    VALUES ('$forum', '$topic_id', '$userR[0]', $time_actu, '1')");
                } else {
                    $r = sql_query("UPDATE " . sql_table('forum_read') . " 
...
if (($op == "solved") and ($topic_id) and ($forum) and ($sec_clef)) {
    if ($user) {
        ...
        if ($local_sec_clef == $sec_clef) {
            $sqlS = "UPDATE " . sql_table('forumtopics') . " 
...
if (($myrow['forum_type'] == 1) and (($myrow['forum_name'] != $forum_name) or ($Forum_passwd != $myrow['forum_pass']))) {
    ...
    for ($i = 0; $i < count($moderator_data); $i++) {
        ...
        if ($modera['user_avatar'] != '') {
            if (stristr($modera['user_avatar'], "users_private")) {
                ...
                $imgtmp = ($ibid = Theme::theme_image("forum/avatar/" . $modera['user_avatar'])) 
                    ? $ibid 
    ...
    if ($forum_access != 9) {
        ...
        if ($forum_access == 2) {
            if (!User::user_is_moderator($userR[0], $userR[2], $forum_access)) {
                $allow_to_post = false;
            }
        }
    ...
    $sql = "SELECT * 
            ...
            ORDER BY topic_first, topic_time DESC 
    ...
    if ($myrow = sql_fetch_assoc($result)) {
        ...
        do {
            ...
            $replys = User::get_total_posts($forum, $myrow['topic_id'], "topic", $Mmod);
            $replys--;
            ...
            if ($replys >= 0) {
                ...
                if ($Sredirection) {
                    ...
                    if ($myrow['topic_poster'] == 1)
                        ...
                        if ($rowQ1) {
                            echo '<td>' . Theme::userpopover($rowQ1[0]['uname'], 40, 2) . $rowQ1[0]['uname'] . '</td>';
                        } else {
