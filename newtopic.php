...
$moderatorX = User::get_moderator($myrow['forum_moderator']);
...
if ($submitS) {
    ...
    if (!isset($user)) {
        ...
        $modo = User::user_is_moderator($userdata['uid'], $userdata['uname'], $forum_access);
        include('header.php');
    }
    ...
    if ($stop != 1) {
        ...
        if (!Spam::R_spambot($asb_question, $asb_reponse, $message)) {
            Log::Ecr_Log('security', 'Forum Anti-Spam : forum=' . $forum . ' / topic_title=' . $subject, '');
        ...
        */
        ...
        $sql = "INSERT INTO " . sql_table('forumtopics') . " (topic_title, topic_poster, current_poster, forum_id, topic_time, topic_notify) 
        ...
        $sql = "INSERT INTO " . sql_table('posts') . " (topic_id, image, forum_id, poster_id, post_text, post_time, poster_ip, poster_dns) 
    ...
    for ($i = 0; $i < count($moderator_data); $i++) {
        $modera = User::getUserData($moderator_data[$i]);
