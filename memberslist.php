 ...
 *
 * @param   [type]  $letter  [$letter description]
 *
 * @return  [type]           [return description]
 */
...
$letter = Hack::removeHack(stripslashes(htmlspecialchars($letter, ENT_QUOTES, cur_charset)));
...
$result = sql_query("SELECT u.uname, u.user_avatar 
                     FROM " . sql_table('users') . " AS u 
                     LEFT JOIN " . sql_table('users_status') . " AS us 
                     ON u.uid = us.uid where us.open='1' 
                     ORDER BY u.uid DESC 
                     LIMIT 0, 1");
...
$count = "SELECT COUNT(uid) AS total 
          FROM " . sql_table('users') . " ";

$select = "SELECT uid, name, uname, femail, url, user_regdate, user_from, email, is_visible, user_viewemail, user_avatar, mns, user_lastvisit 
           FROM " . sql_table('users') . " ";
...
if (($letter != translate("Autres")) and ($letter != translate("Tous"))) {
    ...
    if ($admin and (preg_match('#^[_\.0-9a-z-]+@[0-9a-z-\.]+\.+[a-z]{2,4}$#i', $letter))) {
        $where = "WHERE uname LIKE '" . $letter . "%' 
                  OR email LIKE '%" . strtolower($letter) . "%'" . str_replace('WHERE', ' AND', $ws_req);
    } else {
...
if ($letter != 'front') {
    ...
    if ($num_rows_per_order > 0) {
        ...
        while ($temp_user = sql_fetch_assoc($result)) {
            ...
            $op_result = sql_query("SELECT open 
                                    FROM " . sql_table('users_status') . " 
            ...
            if (($open_user == 1 and $user) || ($admin)) {
                ...
                if ($ibid_avatar = avatar($temp_user['user_avatar'])) {
                    ...
                    <td><a href="user.php?op=userinfo&amp;uname=' . $temp_user['uname'] . '" title="' . translate("Inscription") . ' : ' . date(translate("dateinternal"), (int)$temp_user['user_regdate']);
                }
                ...
                if ($sortby != 'user_from ASC') {
                    if ($admin) {
                        if (Mailer::isbadmailuser($temp_user['uid']) === true) {
