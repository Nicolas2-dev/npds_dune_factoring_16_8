...
switch ($acc) {
    case "newtopic":
        ...
        */
        ...
        if (($forum_type != 6) and ($forum_type != 5)) {
            $messageP = Str::make_clickable($messageP);
            $messageP = Hack::removeHack($messageP);

            if ($allow_bbcode) {
                $messageP = Media::aff_video_yt($messageP);
            }
        }
    ...
    case 'editpost':
        ...
        $myrow2 = sql_fetch_assoc(sql_query("SELECT forum_type 
...
$theposterdata = User::getUserDataFromId($userdatat[0]);
...
echo '&nbsp;<span style="position:absolute; left:6rem;" class="text-body-secondary"><strong>' . $postername . '</strong></span>
