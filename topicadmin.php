...
// if($mode!='viewip') {
    ...
    if (isset($user)) {
        ...
        $myrow = $rowQ1[0];
        $moderator = explode(' ', User::get_moderator($myrow['forum_moderator']));

        for ($i = 0; $i < count($moderator); $i++) {
            if (($userdata[1] == $moderator[$i])) {
                if (User::user_is_moderator($userdata[0], $userdata[2], $myrow['forum_access'])) {
                    $Mmod = true;
                }
    ...
    if ((isset($Mmod) and $Mmod === true) or ($adminforum == 1)) {
        ...
        switch ($mode) {
            case 'move':
                ...
                if ($result = sql_query($sql)) {
                    if ($myrow = sql_fetch_assoc($result)) {
                        ...
                        echo '<option value="-1">' . translate("Plus de forum") . '</option>';
                    }
                } else {
            ...
            case 'unlock':
                ...
                $sql = "UPDATE " . sql_table('forumtopics') . " 
                        SET topic_status = '0', topic_first='1', topic_title='" . addslashes($topic_title) . "' 
                        WHERE topic_id = '$topic'";
            ...
            case 'first':
                $sql = "UPDATE " . sql_table('forumtopics') . " 
                        SET topic_status = '1', topic_first='0' 
                        WHERE topic_id = '$topic'";
