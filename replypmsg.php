...
if (isset($user)) {
    ...
    $userdata = User::getUserData($userdataX[1]);
    $usermore = User::getUserDataFromId($cookie[0]);
    ...
    if ($submitS) {
        ...
        if (strstr($to_user, ',')) {
            ...
            foreach ($tempo as $to_user) {
                $res = sql_query("SELECT uid, user_langue FROM " . sql_table('users') . " WHERE uname='$to_user'");
                ...
                if (($to_userid != '') and ($to_userid != 1)) {
                    ...
                    if ($subscribe) {
                        ...
                        Mailer::copy_to_email($to_userid, $sujet, stripslashes($message));
    ...
    if ($delete_messages and $msg_id) {
        ...
        foreach ($msg_id as $v) {
            if ($type == 'outbox') {
                $sql = "DELETE FROM " . sql_table('priv_msgs') . " 
                        WHERE msg_id='" . $v . "' 
    ...
    if ($classement) {
        ...
        $sql = "UPDATE " . sql_table('priv_msgs') . " 
                SET dossier='$dossier' 
                WHERE msg_id='$msg_id' 
                AND to_userid='" . $userdata['uid'] . "'";
    ...
    if ($reply || $send || $to_user) {
        ...
        if ($reply) {
            ...
            $fromuserdata = User::getUserDataFromId($row['from_userid']);
            ...
            $touserdata = User::getUserDataFromId($row['to_userid']);
        ...
        if ($submitP) {
            ...
            if ($allow_bbcode) {
                ...
                $Xmessage = Media::aff_video_yt($Xmessage);
            }

            $Xmessage = Str::make_clickable($Xmessage);
        ...
        if ($reply) {
            echo Theme::userpopover($fromuserdata['uname'], 48, 2) . '
        ...
        if ($reply and $message == '') {
            $sql = "SELECT p.msg_text, p.msg_time, u.uname 
                    FROM " . sql_table('priv_msgs') . " p, " . sql_table('users') . " u 
            ...
            if ($result = sql_query($sql)) {
                $row = sql_fetch_assoc($result);
                $text = Smilies::smile($row['msg_text']);
