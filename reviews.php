    ...
    if (!$short_review) {
        ...
        if ($admin) {
            ...
            <div class="form-floating mb-3">
                ...
                <span class="help-block">' . translate("Nom de l'image principale non obligatoire, la mettre dans assets/images/reviews/") . '<span class="float-end" id="countcar_cover_rev"></span></span>
            </div>';
        }
    }
...
function send_review($date, $title, $text, $reviewer, $email, $score, $cover, $url, $url_title, $hits, $id, $asb_question, $asb_reponse)
{
    ...
    if (!$user and !$admin) {
        ...
        if (!Spam::R_spambot($asb_question, $asb_reponse, $text)) {
            Log::Ecr_Log('security', 'Review Anti-Spam : title=' . $title, '');
    ...
    if (($admin) && ($id == 0)) {
        sql_query("INSERT INTO " . sql_table('reviews') . " 
        ...
        sql_query("INSERT INTO " . sql_table('reviews_add') . " 
 ...
 * [f_date description]
 *
 * @param   [type]  $xdate  [$xdate description]
 *
 * @return  [type]          [return description]
 */
...
function mod_review($id)
{
    ...
    if (($id != 0) && ($admin)) {
        ...
        <div class="form-floating mb-3">
            ...
            <span class="help-block">' . translate("Nom de l'image principale non obligatoire, la mettre dans assets/images/reviews/") . '<span class="float-end" id="countcar_cover_modrev"></span></span>
        </div>
        <div class="form-floating mb-3">
        ...
        <script type="text/javascript" src="lib/flatpickr/dist/l10n/' . Language::language_iso(1, '', '') . '.js"></script>
 ...
 *
 * @param   [type]  $id_del  [$id_del description]
 *
 * @return  [type]           [return description]
 */
function del_review($id_del)
{
    ...
    if ($admin) {
        sql_query("DELETE FROM " . sql_table('reviews') . " 
                   WHERE id='$id_del'");

        // commentaires
        if (file_exists("modules/comments/reviews.conf.php")) {
            include("modules/comments/reviews.conf.php");
            sql_query("DELETE FROM " . sql_table('posts') . " 
