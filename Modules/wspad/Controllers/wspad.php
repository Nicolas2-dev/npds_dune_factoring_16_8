            ...
            while (list($modtime, $editedby, $ranq, $verrou) = sql_fetch_row($result2)) {
                if ($act == 0) {
                    if (($auteur == $verrou) or ($verrou == '')) {
                        $aff .= '<th data-halign="center" data-align="right">' . wspad_trans("Actions") . '</th>';
                ...
                if (($auteur == $verrou) or ($verrou == '')) {
                    ...
                    $aff .= '<a class="ms-2 fs-5" href="' . $ThisFile . '&amp;op=relo&amp;page=' . urlencode($page) . '&amp;member=' . $groupe . '&amp;ranq=' . $ranq . '" title="' . wspad_trans("Choisir") . '" data-bs-toggle="tooltip" data-bs-placement="left"><i class="bi bi-hand-index-thumb"></i></a>';
                    
                    // supprimer la révision du ranq x
                    $aff .= '<a class="ms-2 fs-5 text-danger" href="' . $ThisFile . '&amp;op=supp&amp;page=' . urlencode($page) . '&amp;member=' . $groupe . '&amp;ranq=' . $ranq . '" title="' . wspad_trans("Supprimer la révision") . '" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="n-danger-tooltip"><i class="bi bi-trash2-fill"></i></a>';
                    
                    // exporter la révision du ranq x
                    $PopUp = JavaPopUp("modules.php?ModPath=$ModPath&amp;ModStart=export&amp;type=doc&amp;pad=" . Crypt::encrypt($page . "#wspad#" . $groupe . "#wspad#" . $ranq), "NPDS_wspad", 5, 5);
                    $aff .= '<a class="ms-2 fs-5" href="javascript:void(0);" onclick="window.open(' . $PopUp . ');" title="' . wspad_trans("Exporter .doc") . '" data-bs-toggle="tooltip" data-bs-placement="left"><i class="bi bi-filetype-doc"></i></a>';
...
function Page($page, $ranq)
{
    ...
    echo '
    <hr /><h3>' . wspad_trans("Document : ") . '</h3><h4>' . $page . '<span class="text-body-secondary">&nbsp;[ ' . wspad_trans("révision") . ' : ' . $row['ranq'] . ' - ' . $row['editedby'] . ' / ' . date(translate("dateinternal"), $row['modtime'] + ((int)$gmt * 3600)) . ' ] </span> <span class="float-end"><img src="modules/' . $ModPath . '/assets/images/ajax_waiting.gif" id="verrous" title="wspad locks" /></span></h4>
