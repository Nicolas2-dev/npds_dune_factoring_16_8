...
include_once('modules/' . $ModPath . '/language/geoloc.lang-' . $language . '.php');
...
if (Groupe::autorisation(-127)) {
    ...
    if ($geo_ip == 1) {
        $arpostip = array();
        $result_postip = sql_query('SELECT COUNT(*) AS nbpost , poster_ip FROM ' . sql_table('posts') . ' GROUP BY poster_ip');
        ...
        $result_ip = sql_query('SELECT * FROM ' . sql_table('ip_loc') . ' ORDER BY ip_visite DESC');
        ...
        while ($row_ip = sql_fetch_array($result_ip)) {
            ...
            if ($ip_lt != 0 and $ip_lg != 0) {
                ...
                switch ($ip_spam) {
                    ...
                    case '5':
                        $infospam = '<span class="small badge bg-danger rounded-pill float-end tooltipbyclass" title="' . geoloc_translate("Adresse IP bannie !") . '">' . $ip_spam . '</span>';
                        break;

                    default:
                        $infospam = '<span class="small badge bg-danger rounded-pill float-end tooltipbyclass" title="' . geoloc_translate("Adresse IP signalée par l’antispam !") . '">' . $ip_spam . '</span>';
                }
...
if (isset($cookie)) {
    $result = sql_query('SELECT uid FROM ' . sql_table('users') . ' WHERE uname LIKE "' . $username . '"');
    $row = sql_fetch_array($result);

    $uid = $row['uid'];

    // voir si user existe dans users_extend
    $resul = sql_query('SELECT uid FROM ' . sql_table('users_extend') . ' WHERE uid = "' . $uid . '"');
    $found = sql_num_rows($resul);

    //mise à jour users_extend si besoin
    if ($found == 0)
        $res = sql_query("INSERT INTO " . sql_table('users_extend') . " VALUES ('$uid','','','','','','','','','','','','','')");
}

//==> georeferencement utilisateur
if (array_key_exists('mod', $_GET)) {
    if ($f_new_lat != '' and $f_new_long != '' and $f_geomod = "neo")
        sql_query('UPDATE ' . sql_table('users_extend') . ' SET ' . $ch_lat . ' = "' . $f_new_lat . '", ' . $ch_lon . ' = "' . $f_new_long . '" WHERE uid = "' . $uid . '"');
    
    if ($f_new_lat != '' and $f_new_long != '' and $f_geomod = "mod")
        sql_query('UPDATE ' . sql_table('users_extend') . ' SET ' . $ch_lat . ' = "' . $f_new_lat . '", ' . $ch_lon . ' = "' . $f_new_long . '" WHERE uid = "' . $f_uid . '"');
}

$result = sql_query('SELECT * FROM ' . sql_table('users') . ' u LEFT JOIN ' . sql_table('users_extend') . ' ue ON u.uid = ue.uid WHERE uname LIKE "' . $username . '"');
...
// ==> membre connecté non géoréférencé
$mbcng = 0; 

// ==> anonyme connectés
$ac = 0; 

// ==> anonyme connectés non géoréférencés
$acng = 0; 

// ==> anonyme connectés géoréférencés
$acg = 0;

$mbcg = 0;

//nombre total on line
$result_con = sql_query('SELECT * FROM ' . sql_table('session') . ' s');
$total_connect = sql_num_rows($result_con);

// récupération des infos des connectés (membres et ano)
$result = sql_query('SELECT * FROM ' . sql_table('session') . ' s
LEFT JOIN ' . sql_table('users') . ' u ON s.username = u.uname
LEFT JOIN ' . sql_table('users_extend') . ' ue ON u.uid = ue.uid');
...
while ($row = sql_fetch_array($result)) {
    ...
    if (!$short_user) {
        include_once('modules/reseaux-sociaux/Config/reseaux-sociaux.conf.php');
    ...
    $tres = sql_query("SELECT * FROM " . sql_table('ip_loc') . " i WHERE ip_ip LIKE \"$session_host_addr\"");
    $r = sql_num_rows($tres);
...
$membre = sql_query('SELECT * FROM ' . sql_table('users') . ' u LEFT JOIN ' . sql_table('users_extend') . ' ue ON u.uid = ue.uid ORDER BY u.uname');
...
foreach ($fond_provider as $k => $v) {
    ...
    switch ($k) {
        case '0':
            $optcart .= '<optgroup label="OpenStreetMap">';
            break;

        case '1':
            $optcart .= '<optgroup label="NASA">';
            break;

        case '2':
            $optcart .= '<optgroup label="Mapbox">';
            break;

        case '4':
            $optcart .= '<optgroup label="Bing maps">';
            break;

        case '7':
            $optcart .= '<optgroup label="Google">';
            break;

        case '8':
            $optcart .= '<optgroup label="ESRI">';
            break;
    }

    $optcart .= '<option ' . $sel . ' value="' . $v[0] . '">' . $v[1] . '</option>';
...
if (!defined('OL')) {
    ...
    $ecr_scr = '<script type="text/javascript" src="' . $nuke_url . '/assets/shared/ol/ol.js"></script>';
}
...
$ecr_scr .= '
    ...
    if (!$("link[href=\'modules/geoloc/assets/css/geoloc_style.css\']").length)
...
if (Groupe::autorisation(-127) and $geo_ip == 1)
    $ecr_scr .= '
...
//<==
$ecr_scr .= 'document.getElementById("mess_info").innerHTML = \'' . $mess_adm . '\';';
...
if ($op == 'allip' and $geo_ip == 1 and Groupe::autorisation(-127))
    $ecr_scr .= '
    ip_cluster.setVisible(true);
    grouputilisateurs.getLayers().forEach(l=>{l.setVisible(false)});
