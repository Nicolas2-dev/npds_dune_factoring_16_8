...
function vidip()
{
    $sql = "DELETE FROM " . sql_table('ip_loc') . " WHERE ip_id >=1";
    if ($result = sql_query($sql))
        sql_query("ALTER TABLE " . sql_table('ip_loc') . " AUTO_INCREMENT = 0;");
}
...
function Configuregeoloc($subop, $ModPath, $ModStart, $ch_lat, $ch_lon, $cartyp, $geo_ip, $api_key_ipdata, $key_lookup)
{
    ...
    $result = sql_query("SELECT CONCAT(ROUND(((DATA_LENGTH + INDEX_LENGTH - DATA_FREE) / 1024 / 1024), 2), ' Mo') AS TailleMo FROM information_schema.TABLES WHERE TABLE_SCHEMA = '$dbname' AND TABLE_NAME = " . sql_table('ip_loc'));
   ...
   <form id="geolocset" name="geoloc_set" action="admin.php" method="post">
    ...
    foreach ($fond_provider as $k => $v) {
        ...
        switch ($k) {
            case '0':
                $aff .= '<optgroup label="OpenStreetMap">';
                break;
            case '1':
                $aff .= '<optgroup label="Mapbox">';
                break;
            case '4':
                $aff .= '<optgroup label="Bing maps">';
                break;
            case '6':
                $aff .= '<optgroup label="Google">';
                break;
            case '7':
                $aff .= '<optgroup label="ESRI">';
                break;
        }
        $aff .= '
