 ...
 *
 * @param   [type]  $iptoshow  [$iptoshow description]
 *
...
function localiser_ip($iptoshow)
{
    ...
    $aff_location = '';
    if ($geo_ip == 1) {
        $ip_location = sql_query("SELECT * FROM " . sql_table('ip_loc') . " WHERE ip_ip LIKE \"" . $iptoshow . "\"");
        ...
        if (sql_num_rows($ip_location) !== 0) {
            ...
            $aff_location .= file_get_contents('modules/geoloc/assets/js/ol-dico.js');
