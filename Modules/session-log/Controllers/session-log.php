...
if ($subop == 'mailog') {
    ...
    Mailer::send_email($adminmail, $subject, $message, $adminmail, true, 'mixed', $file);
}
...
if ($subop == "banthisip") {
    ...
    sql_query("DELETE FROM " . sql_table('session') . " WHERE host_addr='$iptoban'");

    Spam::L_spambot($iptoban, "ban");
    ...
    Url::redirect_url("admin.php?op=Extend-Admin-SubModule&ModPath=" . $ModPath . "&ModStart=" . $ModStart . "&subop=session");
}

// vider la table des sessions
if ($subop == "videsession") {
    sql_query("DELETE FROM " . sql_table('session') . " WHERE host_addr >=0");
