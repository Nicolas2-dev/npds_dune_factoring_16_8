...
if ($tmp = V_secur_cluster($key)) {
    if (($Xop == "NEWS") and ($tmp['SUBSCRIBE'] == "NEWS") and ($tmp['OP'] == "IMPORT")) {
        ...
        $result = sql_query("SELECT name FROM " . sql_table('users') . " WHERE uname='$author'");
        ...
        $result = sql_query("SELECT radminarticle FROM " . sql_table('authors') . " WHERE aid='$aid'");
        ...
        $result = sql_query("SELECT catid FROM " . sql_table('stories_cat') . " WHERE title='" . addslashes($catid) . "'");
        ...
        $result = sql_query("SELECT topicid FROM " . sql_table('topics') . " WHERE topictext='" . addslashes($topic) . "'");
        ...
        if ($pasfinA and $pasfinB) {
            ...
            if ($date_debval == '') {
                $result = sql_query("INSERT INTO " . sql_table('stories') . " VALUES (NULL, '$catid', '$aid', '$subject', now(), '$hometext', '$bodytext', '0', '0', '$topicid', '$author', '$notes', '$ihome', '0', '$date_finval','$epur')");
                ...
                $result = sql_query("INSERT INTO " . sql_table('autonews') . " VALUES (NULL, '$catid', '$aid', '$subject', now(), '$hometext', '$bodytext', '$topicid', '$author', '$notes', '$ihome','$date_debval','$date_finval','$epur')");
                Log::Ecr_Log("security", "Cluster Paradise : insert_autonews ($subject - $date_debval - $date_finval) by AID : $aid", "");
            }

            sql_query("UPDATE " . sql_table('users') . " SET counter=counter+1 WHERE uname='$author'");
