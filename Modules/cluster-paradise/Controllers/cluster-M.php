            ...
            || $tmp[$cpt]["FROMCATID"] == $catid)) 
            {
                ...
                if ((strtoupper($tmp[$cpt]["SUBSCRIBE"]) == "NEWS") and (strtoupper($tmp[$cpt]["OP"]) == "EXPORT")) {
                    ...
                    if (isset($tmp[$cpt]["TOCATEG"])) {
                        ...
                        list($Xcatid) = sql_fetch_row(sql_query("select title from " . sql_table('stories_cat') . " where catid='$catid'"));
                    }
                    ...
                    if (isset($tmp[$cpt]["TOTOPIC"])) {
                        ...
                        list($Xtopic) = sql_fetch_row(sql_query("select topictext from " . sql_table('topics') . " where topicid='$topic'"));
                    }
