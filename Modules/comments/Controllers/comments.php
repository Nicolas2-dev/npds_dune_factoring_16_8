...
$result = sql_query("SELECT COUNT(*) AS total 
...
if ($mycount) {
            ...
            if ($myrow['poster_id'] !== '0') {
                if ($posterdata['user_avatar'] != '') {
                    if (stristr($posterdata['user_avatar'], "users_private")) {
                        ...
                        $imgtmp = $ibid = Theme::theme_image("forum/avatar/" . $posterdata['user_avatar']) 
                            ? $ibid 
    ...
    if ($allow_to_post) {
        echo '<nav class="text-end mb-2">' . Caff_pub($topic, $file_name, $archive) . '</nav>';
    }

    echo '<blockquote class="blockquote my-3">' . translate("Les commentaires sont la propriété de leurs auteurs. Nous ne sommes pas responsables de leur contenu.") . '</blockquote>';
