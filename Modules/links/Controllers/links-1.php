...
function AddLink()
{
    global $ModPath, $ModStart, $links_DB, $links_anonaddlinklock, $op;
    ...
    if (Groupe::autorisation($links_anonaddlinklock)) {
        echo '
        ...
        if ($links_topic) {
            ...
            $toplist = sql_query("SELECT topicid, topictext FROM " . sql_table('topics') . " ORDER BY topictext");
...
function Add($title, $url, $name, $cat, $description, $email, $topicL, $asb_question, $asb_reponse)
{
    ...
    if (!$user and !$admin) {
        ...
        if (!Spam::R_spambot($asb_question, $asb_reponse, '')) {
            Log::Ecr_Log('security', 'Links Anti-Spam : url=' . $url, '');
    ...
    $title = Hack::removeHack(stripslashes(Str::FixQuotes($title)));
    $url = Hack::removeHack(stripslashes(Str::FixQuotes($url)));
    $description = DataImage::dataimagetofileurl($description, 'modules/upload/upload/lindes');
    $description = Hack::removeHack(stripslashes(Str::FixQuotes($description)));
    $name = Hack::removeHack(stripslashes(Str::FixQuotes($name)));
    $email = Hack::removeHack(stripslashes(Str::FixQuotes($email)));
...
function links_search($query, $topicL, $min, $max, $offset)
{
    ...
    $query = Hack::removeHack(stripslashes(htmlspecialchars($query, ENT_QUOTES, cur_charset))); // Romano et NoSP
