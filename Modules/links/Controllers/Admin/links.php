...
if (Groupe::autorisation(-127)) {
    $result = sql_query("SELECT radminsuper FROM " . sql_table('authors') . " WHERE aid='$aid'");
...
function links()
{
    global $ModPath, $ModStart, $links_DB, $admin, $language, $hlpfile;
    ...
    */
    ...
    if ($numrows > 0) {
        ...
        if ($links_topic) {
            ...
            while (list($topicid, $topics) = sql_fetch_row($toplist)) {
                echo '<option value="' . $topicid . '">' . Language::aff_langue($topics) . '</option>';
            }
...
function LinksAddLink($new, $lid, $title, $url, $cat, $description, $name, $email, $submitter, $topicL)
{
    ...
    if ($new == 1) {
        ...
        if ($email != '') {
            ...
            Mailer::send_email($email, $subject, $message, '', false, 'html', '');
        }
    }
...
function LinksModLink($lid, $modifylinkrequest_adv_infos)
{
    ...
    while (list($cid, $sid, $title, $url, $description, $name, $email, $hits, $topicid_card) = sql_fetch_row($result)) {
        ...
        if ($links_topic) {
            ...
            echo '<option value="0">' . translate("Tous les sujets") . '</option>';
...
function LinksListModRequests()
{
    ...
    while (list($requestid, $lid, $cid, $sid, $title, $url, $description, $modifysubmitter, $topicid_card) = sql_fetch_row($resultX)) {
        ...
        $result9 = sql_query("SELECT topictext FROM " . sql_table('topics') . " WHERE topicid='$oritopicid_card'");

        $result9b = sql_query("SELECT topictext FROM " . sql_table('topics') . " WHERE topicid='$topicid_card'");
