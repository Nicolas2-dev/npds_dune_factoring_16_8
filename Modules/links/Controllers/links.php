...
if ($links_DB == '') $links_DB = sql_table('');
...
function autorise_mod($lid, $aff)
{
    ...
    if ($admin) {
        ...
        $result = sql_query("SELECT radminsuper FROM " . sql_table('authors') . " WHERE aid='$Xadmin[0]'");
...
function viewlinkeditorial($lid, $ttitle)
{
    ...
    <h3>' . translate("EDITO") . ' : 
        <span class="text-body-secondary">' . Language::aff_langue($displaytitle) . '</span>';
