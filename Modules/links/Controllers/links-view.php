...
while (list($lid, $url, $title, $description, $time, $hits, $topicid_card, $xcid, $xsid) = sql_fetch_row($result)) {
    ...
    if ($affich) {
        ...
        if ($url == '')
            echo '
               <h4 class="text-body-secondary"><i class="fas fa-external-link-alt"></i>&nbsp;' . Language::aff_langue($title);
        else
            echo '
        ...
        if ($links_topic and $topicid_card != 0) {
            list($topicLX) = sql_fetch_row(sql_query("SELECT topictext FROM " . sql_table('topics') . " WHERE topicid='$topicid_card'"));
