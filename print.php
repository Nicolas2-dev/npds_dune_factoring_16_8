 ...
 *
 * @param   [type]  $oper  [$oper description]
 * @param   [type]  $DB    [$DB description]
 * @param   [type]  $nl    [$nl description]
...
function PrintPage($oper, $DB, $nl, $sid)
{
    ...
    if ($oper == 'links') {
        $DB = Hack::removeHack(stripslashes(htmlentities(urldecode($DB), ENT_NOQUOTES, cur_charset)));
    ...
    if ($aff == true) {
        ...
        if (($oper == 'news') or ($oper == 'archive')) {
            ...
            echo '
                <span class="float-end" style="font-size: .8rem;"> ' . Date::formatTimestamp($time) . '</span><br />
                <hr />
        ...
        if ($oper == 'links') {
            echo '<span class="float-end" style="font-size: .8rem;">' . Date::formatTimestamp($time) . '</span><br /><hr />';
