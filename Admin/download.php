...
function DownloadAdmin()
{
    ...
    while (list($dcategory) = sql_fetch_row($resultX)) {
        ...
        <h4 class="mb-2"><a class="tog" id="show_cat_' . $pseudocatid . '" title="Déplier la liste"><i id="i_cat_' . $pseudocatid . '" class="fa fa-caret-down fa-lg text-primary"></i></a>
            ' . Language::aff_langue(stripslashes($dcategory)) . '</h4>';
        ...
        $result = sql_query("SELECT did, dcounter, durl, dfilename, dfilesize, ddate, dver, perms 
                             FROM " . sql_table('downloads') . " 
    ...
    $result = sql_query("SELECT DISTINCT dcategory 
                         FROM " . sql_table('downloads') . " 
                         ORDER BY dcategory");
 ...
 * @param   [type]  $durl         [$durl description]
 * @param   [type]  $dfilename    [$dfilename description]
 * @param   [type]  $dfilesize    [$dfilesize description]
 * @param   [type]  $dweb         [$dweb description]
 * @param   [type]  $duser        [$duser description]
 * @param   [type]  $ddate        [$ddate description]
 * @param   [type]  $dver         [$dver description]
 * @param   [type]  $dcategory    [$dcategory description]
 * @param   [type]  $sdcategory   [$sdcategory description]
 * @param   [type]  $description  [$description description]
 * @param   [type]  $privs        [$privs description]
 * @param   [type]  $Mprivs       [$Mprivs description]
 *
 * @return  [type]                [return description]
 */
function DownloadSave($did, $dcounter, $durl, $dfilename, $dfilesize, $dweb, $duser, $ddate, $dver, $dcategory, $sdcategory, $description, $privs, $Mprivs)
{
    ...
    if ($ddate == "yes") {
        ...
        sql_query("UPDATE " . sql_table('downloads') . " SET dcounter='$dcounter', durl='$durl', dfilename='$dfilename', dfilesize='$dfilesize', ddate='$time', dweb='$dweb', duser='$duser', dver='$dver', dcategory='$dcategory', ddescription='$description', perms='$privs' WHERE did='$did'");
    } else {
        sql_query("UPDATE " . sql_table('downloads') . " SET dcounter='$dcounter', durl='$durl', dfilename='$dfilename', dfilesize='$dfilesize', dweb='$dweb', duser='$duser', dver='$dver', dcategory='$dcategory', ddescription='$description', perms='$privs' WHERE did='$did'");
    }
...
function DownloadAdd($dcounter, $durl, $dfilename, $dfilesize, $dweb, $duser, $dver, $dcategory, $sdcategory, $description, $privs, $Mprivs)
{
    ...
    if (($durl) and ($dfilename)) {
        sql_query("INSERT 
                   INTO " . sql_table('downloads') . " 
                   VALUES ('0', '0', '$durl', '$dfilename', '0', '$time', '$dweb', '$duser', '$dver', '$dcategory', '$description', '$privs')");
    }
