...
function displayadmins()
{
    ...
    while (list($a_aid, $name, $url, $email, $supadm) = sql_fetch_row($result)) {
        ...
        if ($url != '') {
            echo '<a href="' . $url . '"><i class="fas fa-external-link-alt fa-lg" title="' . adm_translate("Visiter le site web") . '" data-bs-toggle="tooltip"></i></a>&nbsp;';
        }
...
function modifyadmin($chng_aid)
{
    ...
    $result = sql_query("SELECT aid, name, url, email, pwd, radminsuper 
                         FROM " . sql_table('authors') . " 
                         WHERE aid='$chng_aid'");
    ...
    $result3 = sql_query("SELECT * 
                          FROM " . sql_table('droits') . " 
                          WHERE d_aut_aid ='$chng_aid'");
...
function updateadmin($chng_aid, $chng_name, $chng_email, $chng_url, $chng_radminsuper, $chng_pwd, $chng_pwd2, $ad_d_27, $old_pwd)
{
    ...
    if ($chng_pwd2 != '') {
        ...
        $result = $chng_radminsuper == 1 
            ? sql_query("UPDATE " . sql_table('authors') . " 
                         SET name='$chng_name', email='$chng_email', url='$chng_url', radminsuper='$chng_radminsuper', pwd='$chng_pwd', hashkey='1' 
                         WHERE aid='$chng_aid'") 

            : sql_query("UPDATE " . sql_table('authors') . " 
                         SET name='$chng_name', email='$chng_email', url='$chng_url', radminsuper='0', pwd='$chng_pwd', hashkey='1' 
                         WHERE aid='$chng_aid'");
            
    } else {
        if ($chng_radminsuper == 1) {
            $result = sql_query("UPDATE " . sql_table('authors') . " 
    ...
    global $aid;
    Log::Ecr_Log('security', "ModifyAuthor($chng_name) by AID : $aid", '');
...
switch ($op) {
    ...
    case 'AddAuthor':
        ...
        global $aid;
        Log::Ecr_Log('security', "AddAuthor($add_aid) by AID : $aid", '');
    ...
    case 'deladminconf':
        ...
        global $aid;
        Log::Ecr_Log('security', "DeleteAuthor($del_aid) by AID : $aid", '');
