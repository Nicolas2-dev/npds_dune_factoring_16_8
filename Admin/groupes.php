    ...
    if ($one_gp) {
        ...
        foreach ($tab_groupeIII as $bidon => $gp) {
            ...
            <div id="bloc_gr_' . $gp . '" class="row border rounded ms-1 p-2 px-0 mb-2 w-100">
                <div class="col-lg-4 ">
                <span>' . $gp . '</span>
                <i class="fa fa-users fa-2x text-body-secondary"></i><h4 class="my-2">' . Language::aff_langue($result['groupe_name']) . '</h4><p>' . Language::aff_langue($result['groupe_description']);
            ...
            foreach ($tab_groupe as $bidon => $uidX) {
                if ($uidX) {
                    ...
                    list($uname, $user_avatar) = sql_fetch_row(sql_query("SELECT uname, user_avatar 
                    ...
                    if ($result['groupe_forum'] == 1) {
                        $pat = '#\b' . $uidX . '\b#';
                        $res = sql_query("SELECT f.forum_id, f.forum_name, f.forum_moderator 
                                          FROM " . sql_table('forums') . " f 
                                          WHERE f.forum_pass='$gp'");
    ...
    while (list($gp, $gp_name, $gp_description) = sql_fetch_row($result)) {
        ...
        if ($gpA) {
            ...
            <div class="row border rounded ms-1 p-2 px-0 mb-2 w-100">
                ...
                <h4 class="my-2 text-body-secondary">' . Language::aff_langue($gp_name) . '</h4>
                <p class="text-body-secondary">' . Language::aff_langue($gp_description);
...
function membre_add($gp)
{
    ...
    echo (mysqli_get_client_info() <= '8.0') ?
        Js::auto_complete_multi('membre', 'uname', 'users', 'luname', 'inner join users_status on users.uid=users_status.uid WHERE users.uid<>1 AND groupe NOT REGEXP \'[[:<:]]' . $gp . '[[:>:]]\'') :
        Js::auto_complete_multi('membre', 'uname', 'users', 'luname', 'inner join users_status on users.uid=users_status.uid WHERE users.uid<>1 AND groupe NOT REGEXP \'\\b' . $gp . '\\b\'');
 ...
 *
 * @param   [type]  $groupe_id  [$groupe_id description]
 * @param   [type]  $luname     [$luname description]
 *
 * @return  [type]              [return description]
 */
function membre_add_finish($groupe_id, $luname)
{
    ...
    for ($j = 0; $j < $nbremembres; $j++) {
        ...
        if ($ibid['uid']) {
            ...
            $result2 = sql_query("SELECT groupe 
                                  FROM " . sql_table('users_status') . " 
                                  WHERE uid='" . $ibid['uid'] . "'");
            ...
            if (!$groupeexistedeja) {
                ...
                $resultat = sql_query("UPDATE " . sql_table('users_status') . " 
    ...
    global $aid;
    Log::Ecr_Log('security', "AddMemberToGroup($groupe_id, $luname) by AID : $aid", '');
    ...
    if ($ok == 0) {
        ...
        while ($r = sql_fetch_row($res)) {
            ...
            sql_query("UPDATE " . sql_table('forums') . " 
                       SET forum_moderator='$new_moder' 
                       WHERE forum_id='$r[0]'");
        }

        $resultat = sql_query("SELECT groupe 
                               FROM " . sql_table('users_status') . " 
                               WHERE uid='$uid'");
        ...
        global $aid;
        Log::Ecr_Log('security', "DeleteMemberToGroup($groupe_id, $uname) by AID : $aid", '');
    }
 ...
 *
 * @param   [type]  $groupe_id   [$groupe_id description]
 * @param   [type]  $tab_groupe  [$tab_groupe description]
 *
 * @return  [type]               [return description]
 */
function retiredugroupe_all($groupe_id, $tab_groupe)
{
    ...
    foreach ($tab_groupe as $bidon => $uidZ) {
        if ($uidZ) {
            ...
            global $aid;
            Log::Ecr_Log('security', "DeleteAllMemberToGroup($groupe_id, $uidZ) by AID : $aid", '');
        }
    }
 ...
 *
 * @param   [type]  $sub_op  [$sub_op description]
 *
 * @return  [type]           [return description]
 */
function groupe_maj($sub_op)
{
    ...
    if ($sub_op == adm_translate("Sauver les modifications")) {
        sql_query("UPDATE " . sql_table('groupes') . " 
                   SET groupe_name='$groupe_name', groupe_description='$groupe_description' 
                   WHERE groupe_id='$groupe_id'");
        
        global $aid;
        Log::Ecr_Log("security", "UpdateGroup($groupe_id) by AID : $aid", '');
    }
...
function groupe_delete($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "DeleteGroup($groupe_id) by AID : $aid", '');
}
...
function workspace_create($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "CreateWS($groupe_id) by AID : $aid", '');
}
...
function pad_create($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "CreatePadWS($groupe_id) by AID : $aid", '');
}
...
function pad_remove($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "DeletePadWS($groupe_id) by AID : $aid", '');
}
...
{
    $sql = "CREATE TABLE IF NOT EXISTS " . sql_table('blocnotes') . " (
    ...
    sql_query("UPDATE " . sql_table('groupes') . " 
               SET groupe_blocnote = '1' 
               WHERE groupe_id = '$groupe_id';");

    global $aid;
    Log::Ecr_Log('security', "CreateBlocnoteWS($groupe_id) by AID : $aid", '');
}
...
function note_remove($groupe_id)
{
    ...
    sql_query("UPDATE " . sql_table('groupes') . " 
               SET groupe_blocnote = '0' 
               WHERE groupe_id = '$groupe_id';");

    global $aid;
    Log::Ecr_Log('security', "DeleteBlocnoteWS($groupe_id) by AID : $aid", '');
}
...
function workspace_archive($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "ArchiveWS($groupe_id) by AID : $aid", '');
}
 ...
 *
 * @param   [type]  $groupe_id    [$groupe_id description]
 * @param   [type]  $groupe_name  [$groupe_name description]
 * @param   [type]  $description  [$description description]
 * @param   [type]  $moder        [$moder description]
 *
 * @return  [type]                [return description]
 */
function forum_groupe_create($groupe_id, $groupe_name, $description, $moder)
{
    ...
    $result = sql_query("SELECT cat_id 
    ...
    if (!$cat_id) {
        sql_query("INSERT 
                   ...
                   VALUES (-1, '" . adm_translate("Groupe de travail") . "')");
    }
    //==>creation forum
    ...
    sql_query("INSERT 
               ...
               VALUES (NULL, '$groupe_name', '$description', '1', '$moder', '-1', '7', '$groupe_id', '0', '0', '0')");
    ...
    global $aid;
    Log::Ecr_Log("security", "CreateForumWS($groupe_id) by AID : $aid", '');
}
 ...
 * @param   [type]  $forum_moderator  [$forum_moderator description]
 *
 * @return  [type]                    [return description]
 */
...
{
    sql_query("UPDATE " . sql_table('forums') . " 
               SET forum_moderator = '$forum_moderator' 
...
function forum_groupe_delete($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "DeleteForumWS($forum_id) by AID : $aid", '');
}
...
function groupe_mns_create($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "CreateMnsWS($groupe_id) by AID : $aid", '');
}
...
function groupe_mns_delete($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "DeleteMnsWS($groupe_id) by AID : $aid", '');
}
...
function groupe_chat_create($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "ActivateChatWS($groupe_id) by AID : $aid", '');
}
...
function groupe_chat_delete($groupe_id)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "DesactivateChatWS($groupe_id) by AID : $aid", '');
}
...
function bloc_groupe_create($groupe_id)
{
    ...
    $row = sql_fetch_row(sql_query("SELECT COUNT(id) 
                                    FROM " . sql_table('lblocks') . " 
                                    WHERE content='$menu_workspace'"));
    ...
    if ($bloc == false) {
        sql_query("INSERT 
                   ...
                   VALUES (NULL, '', '$menu_workspace', '$groupe_id', '3', '0', '1', '0', NULL)");
    }
}
...
function groupe_member_ask()
{
    ...
    if (isset($sub_op)) {
        ...
        if ($sub_op == adm_translate("Oui")) {
            ...
            global $aid;
            Log::Ecr_Log('security', "AddMemberToGroup($groupe_asked, $uname) by AID : $aid", '');
    ...
    foreach ($iterator as $fileinfo) {
        if ($fileinfo->isFile() and strpos($fileinfo->getFilename(), 'ask4group') !== false) {
            ...
            $myrow = User::getUserDataFromId($us_gr[1]);

            $r = sql_query("SELECT groupe_name 
                            FROM " . sql_table('groupes') . " 
                            WHERE groupe_id='" . $us_gr[2] . "'");
...
switch ($op) {
    ...
    case 'groupe_add_finish':
        ...
        if (($groupe_id == '') or ($groupe_id < 2) or ($groupe_id > 126)) {
            $row = sql_fetch_row(sql_query("SELECT MAX(groupe_id) 
                                            FROM " . sql_table('groupes')));
        ...
        if ($ok_grp) {
            ...
            global $aid;
            Log::Ecr_Log('security', "CreateGroupe($groupe_id, $groupe_name) by AID : $aid", '');
        }
    default:
        group_liste();
        break;
