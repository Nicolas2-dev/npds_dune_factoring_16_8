...
function EditCategory($catid)
{
    ...
    if (!$catid) {
        ...
        while (list($catid, $title) = sql_fetch_row($selcat)) {
            echo '<option name="catid" value="' . $catid . '">' . Language::aff_langue($title) . '</option>';
        }
...
function DelCategory($cat)
{
    ...
    if (!$cat) {
        ...
        while (list($catid, $title) = sql_fetch_row($selcat)) {
            echo '
               <option name="cat" value="' . $catid . '">' . Language::aff_langue($title) . '</option>';
        }
...
function YesDelCategory($catid)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "YesDelCategory($catid) by AID : $aid", '');
...
function previewStory($qid, $uid, $author, $subject, $hometext, $bodytext, $topic, $notes, $catid, $ihome, $members, $Mmembers, $dd_pub, $fd_pub, $dh_pub, $fh_pub, $epur)
{
    ...
    $notes = stripslashes(DataImage::dataimagetofileurl($notes, 'cache/an'));
...
function postStory($type_pub, $qid, $uid, $author, $subject, $hometext, $bodytext, $topic, $notes, $catid, $ihome, $members, $Mmembers, $date_debval, $date_finval, $epur)
{
    ...
    $hometext = DataImage::dataimagetofileurl($hometext, 'modules/upload/upload/ai');
    $bodytext = DataImage::dataimagetofileurl($bodytext, 'modules/upload/upload/ac');
    $notes = DataImage::dataimagetofileurl($notes, 'modules/upload/upload/an');
...
function editStory($sid)
{
    ...
    echo Editeur::fetch("hometext", "true");
    ...
    echo Editeur::fetch("bodytext", "true");
    ...
    echo '
        <div class="mb-3 row">
            ...
            <div class="col-sm-6 my-2">
                ...
                <span class="small help-block">' . translate(date("l")) . date(" " . translate("dateinternal"), time() + ((int)$gmt * 3600)) . '</span>
            </div>
        </div>
        <div class="mb-3 row">
...
function changeStory($sid, $subject, $hometext, $bodytext, $topic, $notes, $catid, $ihome, $members, $Mmembers, $Cdate, $Csid, $date_finval, $epur, $theme, $dd_pub, $fd_pub, $dh_pub, $fh_pub)
{
    ...
    global $aid;
    Log::Ecr_Log('security', "changeStory($sid, $subject, hometext..., bodytext..., $topic, notes..., $catid, $ihome, $members, $Mmembers, $Cdate, $Csid, $date_finval,$epur,$theme) by AID : $aid", '');
