...
function email_user()
{
    ...
    $resultID = sql_query("SELECT groupe_id, groupe_name 
 ...
 * @param   [type]  $all         [$all description]
 * @param   [type]  $groupe      [$groupe description]
 * @param   [type]  $expediteur  [$expediteur description]
 *
 * @return  [type]               [return description]
 */
function send_email_to_user($username, $subject, $message, $all, $groupe, $expediteur)
{
    ...
    if ($subject != '') {
        ...
        if ($all) {
            $result = sql_query("SELECT uid, user_langue 
            ...
            if ($groupe) {
                $result = sql_query("SELECT s.uid, s.groupe, u.user_langue 
                                     FROM " . sql_table('users_status') . " s, " . sql_table('users') . " u 
                                     WHERE s.uid=u.uid 
                                     AND s.groupe!='' 
                                     ORDER BY s.uid ASC");
        ...
        while ($count < sizeof($tab_to_userid)) {
            ...
            if (($to_userid != '') and ($to_userid != 1)) {
                $sql = "INSERT 
                        ...
                        VALUES ('$image', '$subject', '$emetteur', '$to_userid', '$time', '$message')";
    ...
    global $aid;
    Log::Ecr_Log('security', "SendEmailToUser($subject) by AID : $aid", '');
