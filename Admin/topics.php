...
{
    global $hlpfile, $tipath, $f_meta_nom, $f_titre, $nook;
...
{
    global $hlpfile, $tipath, $f_meta_nom, $f_titre;
    ...
    echo '
    <hr />
    <h3 class="mb-3">' . adm_translate("Editer le Sujet :") . ' <span class="text-body-secondary">' . Language::aff_langue($topicname) . '</span></h3>';
...
function topicchange($topicid, $topicname, $topicimage, $topictext, $topicadmin, $name, $url)
{
    ...
    foreach ($topad as $value) { 
        ...
        //si le nom de l'adminsujet n'est pas dans les nouveaux adminsujet
        if (!in_array($value, $topicadminX)) {
