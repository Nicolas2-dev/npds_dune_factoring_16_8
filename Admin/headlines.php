...
function HeadlinesAdmin()
{
    ...
    $result = sql_query("SELECT hid, sitename, url, headlinesurl, status 
...
{
    sql_query("UPDATE " . sql_table('headlines') . " 
               SET sitename='$xsitename', url='$url', headlinesurl='$headlinesurl', status='$status' 
               WHERE hid='$hid'");
...
{
    sql_query("INSERT 
               INTO " . sql_table('headlines') . " 
               VALUES (NULL, '$xsitename', '$url', '$headlinesurl', '$status')");
