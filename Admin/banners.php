...
function BannersAdmin()
{
    ...
    $result = sql_query("SELECT bid, cid, imageurl, imptotal, impmade, clicks, date 
                         FROM " . sql_table('banner') . " 
                         WHERE userlevel!='9' 
                         ORDER BY bid");

    while (list($bid, $cid, $imageurl, $imptotal, $impmade, $clicks, $date) = sql_fetch_row($result)) {
        $result2 = sql_query("SELECT cid, name 
    ...
    $result = sql_query("SELECT bid, cid, impressions, clicks, datestart, dateend 
    ...
    $result = sql_query("SELECT cid, name, contact, email 
                         FROM " . sql_table('bannerclient') . " 
                         ORDER BY cid");

    while (list($cid, $name, $contact, $email) = sql_fetch_row($result)) {
        $result2 = sql_query("SELECT cid 
                              FROM " . sql_table('banner') . " 
                              WHERE cid='$cid'");
 ...
 * @param   [type]  $imptotal   [$imptotal description]
 * @param   [type]  $imageurl   [$imageurl description]
 * @param   [type]  $clickurl   [$clickurl description]
 * @param   [type]  $userlevel  [$userlevel description]
 *
 * @return  [type]              [return description]
 */
 ...
 * @param   [type]  $cid        [$cid description]
 * @param   [type]  $imptotal   [$imptotal description]
 * @param   [type]  $impadded   [$impadded description]
 * @param   [type]  $imageurl   [$imageurl description]
 * @param   [type]  $clickurl   [$clickurl description]
 * @param   [type]  $userlevel  [$userlevel description]
 *
...
function BannerChange($bid, $cid, $imptotal, $impadded, $imageurl, $clickurl, $userlevel)
{
    ...
    sql_query("UPDATE " . sql_table('banner') . " 
               SET cid='$cid', imptotal='$imp', imageurl='$imageurl', clickurl='$clickurl', userlevel='$userlevel' 
               WHERE bid='$bid'");
...
{
    sql_query("UPDATE " . sql_table('bannerclient') . " 
