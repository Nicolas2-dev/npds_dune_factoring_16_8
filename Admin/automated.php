...
function autoStory()
{
    ...
    $result = sql_query("SELECT anid, title, date_debval, topic 
                         FROM " . sql_table('autonews') . " 
                         ORDER BY date_debval ASC");
 ...
 *
 * @param   [type]  $anid  [$anid description]
 *
 * @return  [type]         [return description]
 */
...
function autoEdit($anid)
{
    ...
    $result2 = sql_query("SELECT topicname, topictext, topicimage, topicadmin 
                          FROM " . sql_table('topics') . " 
                          WHERE topicid='$topic'");
...
function autoSaveEdit($anid, $title, $hometext, $bodytext, $topic, $notes, $catid, $ihome, $informant, $members, $Mmembers, $date_debval, $date_finval, $epur)
{
    ...
    $result = sql_query("UPDATE " . sql_table('autonews') . " 
                         SET 
                            catid='$catid', 
                            title='$title', 
                            time=now(), 
