...
function show_imm($op)
{
    ...
    if (!$user) {
        ...
        $sql = ($op != 'new_msg') 
            ? "SELECT * FROM " . sql_table('priv_msgs') . " 
        ...
        while ($myrow = sql_fetch_assoc($result)) {
            ...
            $posterdata = User::getUserDataFromId($myrow['from_userid']);

            echo '
                <div class="card mb-3">
                <div class="card-body">
                    <div>' . Theme::userpopover($posterdata['uname'], 40, 2) . ' <span class="float-end small">' . translate("Envoyé") . ' : ' . $myrow['msg_time'] . '</span>';
            ...
            if ($posterdata['uid']<>1) {
                echo User::member_qualif($posterdata['uname'], $posts, $posterdata['rang']);
            }
            */
            echo '<hr />';

            echo '<h4>' . Language::aff_langue($myrow['subject']);
            ...
            $message = str_replace("[addsig]", '<div class="n-signature">' . nl2br($posterdata['user_sig']) . '</div>', Language::aff_langue($message)); // ne sert à rien ici ????
 ...
 * [sup_imm description]
 *
 * @param   [type]  $msg_id  [$msg_id description]
 *
 * @return  [type]           [return description]
 */
