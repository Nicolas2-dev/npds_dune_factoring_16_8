...
use Npds\Support\Facades\CacheEmpty;
use Npds\Support\Facades\CacheManager;
...
if (Config::get('cache.super_cache')) {
    $cache_obj = CacheManager::getInstance();
    $cache_obj->startCachingPage();
} else {
    $cache_obj = CacheEmpty::getInstance();
}

if (($cache_obj->genereting_output == 1) or ($cache_obj->genereting_output == -1) or (!Config::get('cache.super_cache'))) {
    ...
    if ($informant == '') {
        $informant = Config::get('user.anonymous');
    }
    ...
    $boxstuff .= '
        ...
        <ul>
            <li>
                ...
                <span class="h5">
                    <span class="badge bg-secondary" title="' . $topicname . '<hr />' . Language::aff_langue($topictext) . '" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="right">
            ...
            <li>
                ...
                </a> ' . Theme::userpopover($informant, 36, '') . '
            </li>
        </ul>
        <div>
    ...
    if (!function_exists("Caff_pub")) {
        if (file_exists("modules/comments/article.conf.php")) {
            ...
            include("modules/comments/Controllers/comments.php");
        }
    }
}
