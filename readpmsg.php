...
if (!$user) {
    ...
    if (!sql_num_rows($resultID)) {
        ...
        // debile de faire ça !!!!!
        $posterdata = ($type == 'outbox') ? User::getUserDataFromId($myrow['to_userid']) : User::getUserDataFromId($myrow['from_userid']);
        ...
        if ($posterdata['uid'] <> 1) {
            ...
            $my_rs = '';
            if (!$short_user) {
                $posterdata_extend = User::getUserDataExtendFromId($posterdata['uid']);
        ...
        if ($posterdata['user_avatar'] != '') {
            ...
            if ($posterdata['uid'] <> 1) {
                ...
                echo '<a style="position:absolute; top:1rem;" tabindex="0" data-bs-toggle="popover" data-bs-trigger="focus" data-bs-html="true" data-bs-title="' . $posterdata['uname'] . '" data-bs-content=\'' . User::member_qualif($posterdata['uname'], $posts, $posterdata['rang']) . '<br /><div class="list-group">' . $useroutils . '</div><hr />' . $aff_reso . '\'><img class=" btn-secondary img-thumbnail img-fluid n-ava" src="' . $imgtmp . '" alt="' . $posterdata['uname'] . '" /></a>';
            } else {
        ...
        $message = str_replace('[addsig]', '<br /><div class="n-signature">' . nl2br($posterdata['user_sig']) . '</div>', Language::aff_langue($message));
