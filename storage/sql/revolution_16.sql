...
INSERT INTO metalang (def, content, type_meta, type_uri, uri, description, obligatoire) VALUES('!date!', 'function MM_date () {\n   global $gmt;\n   $fmt = datefmt_create(language_iso(1,"_",1),IntlDateFormatter::FULL,IntlDateFormatter::MEDIUM,"Europe/Paris",IntlDateFormatter::GREGORIAN,);\n   $MT_date = datefmt_format($fmt,time()+((integer)$gmt*3600));\n   $MT_date = ucfirst($MT_date);\n   return ($MT_date);\n}', 'meta', '-', NULL, '[french]Date du jour - affectée par la valeur gmt défini dans les préférences[/french]', '1');
