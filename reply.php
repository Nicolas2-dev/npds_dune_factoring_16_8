...
if ($submitS) {
    ...
    if (!isset($user)) {
        if ($forum_access == 0) {
            ...
            if (($username == '') or ($password == '')) {
                ...
                $modo = User::user_is_moderator($username, $pass, $forum_access);
        ...
        $modo = User::user_is_moderator($userdata[0], $userdata[2], $forum_access);
        ...
        $userdata = User::getUserData($userdata[1]);
    ...
    if ($stop != 1) {
        ...
        $hostname = $dns_verif ? @gethostbyaddr(urldecode($poster_ip)) : '';

        // anti flood
        Spam::anti_flood($modo, $anti_flood, $poster_ip, $userdata, $gmt);
        //anti_spambot

        if (!Spam::R_spambot($asb_question, $asb_reponse, $message)) {
            Log::Ecr_Log('security', 'Forum Anti-Spam : forum=' . $forum . ' / topic=' . $topic, '');
        ...
        if (($forum_type != '6') and ($forum_type != '5')) {
            $message = Str::make_clickable($message);
        ...
        $sql = "INSERT INTO " . sql_table('posts') . " (post_idH, topic_id, image, forum_id, poster_id, post_text, post_time, poster_ip, poster_dns) 
        ...
        $sql = "UPDATE " . sql_table('users_status') . " 
                SET posts=posts+1 
        ...
        $sql = "SELECT t.topic_notify, u.email, u.uname, u.uid, u.user_langue 
                FROM " . sql_table('forumtopics') . " t, " . sql_table('users') . " u 
        ...
        if (($m['topic_notify'] == 1) && ($m['uname'] != $userdata['uname'])) {
            ...
            Mailer::send_email($m['email'], $subject, $message, '', true, "html", '');
            $sauf = $m['uid'];
        }
    ...
    list($topic_title, $topic_status) = sql_fetch_row(sql_query("SELECT topic_title, topic_status 
                                                                 FROM " . sql_table('forumtopics') . " 
                                                                 WHERE topic_id='$topic'"));

    // correction bug user non connecter userdata[0] nexiste pas !!!!
    if (isset($user)) {
        $userX = base64_decode($user);
        $userdata = explode(':', $userX);
    } else {
    ...
    $posterdata = User::getUserDataFromId($userdata[0]);
    ...
    $moderator = User::get_moderator($mod);
    ...
    for ($i = 0; $i < count($moderator); $i++) {
        $modera = User::getUserData($moderator[$i]);
    ...
    if ($forum_access == 0){
        ...
        if (User::user_is_moderator($userdata[0], $userdata[2], $forum_access)) {
            $allow_to_reply = true;
        }
    }
    ...
    if ($allow_to_reply) {
        ...
        if ($smilies) {
            ...
            <div class="d-none d-sm-block mb-3 row">
                ...
                <div class="col-sm-12">
                    ...
                    ' . Smilies::emotion_add($image_subject) . '
                    </div>
                </div>
            </div>';
        }
        ...
        if ($citation && !$submitP) {
            $sql = "SELECT p.post_text, p.post_time, u.uname 
                    FROM " . sql_table('posts') . " p, " . sql_table('users') . " u 
                    WHERE post_id = '$post' 
    ...
    if ($allow_to_reply) {
        echo '<h4 class="my-3">' . translate("Aperçu des sujets :") . '</h4>';
        ...
        do {
            ...
            $posterdata = User::getUserDataFromId($myrow['poster_id']);
            ...
            if ($myrow['poster_id'] !== '0') {
                ...
                if (!$short_user) {
                    ...
                    $posterdata_extend = User::getUserDataExtendFromId($myrow['poster_id']);
            ...
            if ($smilies) {
                if ($myrow['poster_id'] !== '0') {
                    ...
                    echo '
                    <a style="position:absolute; top:1rem;" tabindex="0" data-bs-toggle="popover" data-bs-trigger="focus" data-bs-html="true" data-bs-title="' . $posterdata['uname'] . '" data-bs-content=\'<div class="my-2 border rounded p-2">' . User::member_qualif($posterdata['uname'], $posts, $posterdata['rang']) . '</div><div class="list-group mb-3 text-center">' . $useroutils . '</div><div class="mx-auto text-center" style="max-width:170px;">' . $my_rs . '</div>\'><img class=" btn-outline-primary img-thumbnail img-fluid n-ava" src="' . $imgtmp . '" alt="' . $posterdata['uname'] . '" /></a>
            ...
            echo '
               ...
               <div class="card-body">
                  <span class="text-body-secondary float-end small" style="margin-top:-1rem;">' . translate("Posté : ") . Date::convertdate($myrow['post_time']) . '</span>
