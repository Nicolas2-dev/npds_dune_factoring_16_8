...
function GraphicAdmin($hlpfile)
{
    ...
    $alert_modules = sql_query("SELECT * FROM " . sql_table('fonctions') . " f 
                                LEFT JOIN " . sql_table('') . "modules m 
                                ON m.mnom = f.fnom 
                                WHERE m.minstall=1 
                                AND fcategorie=9");
    ...
    if ($hlpfile) {
        $adm_ent .= '<li class="ms-2" data-bs-toggle="tooltip" title="' . adm_translate("Manuel en ligne") . '"><a class="btn btn-outline-primary btn-sm" href="javascript:openwindow();"><i class="fa fa-question-circle fa-2x"></i></a></li>';
    }
