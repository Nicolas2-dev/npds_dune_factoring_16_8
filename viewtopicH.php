...
} else { // correction bug userdata non definie si user non conecter !
    $userdata[0] = 0;
}
...
$total = User::get_total_posts($forum, $topic, "topic", $Mmod);
...
$contributeurs = User::get_contributeurs($forum, $topic);
 ...
 *
 * @param   [type]  $rootcatid  [$rootcatid description]
 * @param   [type]  $sql        [$sql description]
 * @param   [type]  $maxlevel   [$maxlevel description]
 *
 ...
 *
 * @param   [type]  $parcat       [$parcat description]
 * @param   [type]  $table        [$table description]
 * @param   [type]  $level        [$level description]
 * @param   [type]  $maxlevel     [$maxlevel description]
 * @param   [type]  $max_post_id  [$max_post_id description]
 * @param   [type]  $clas         [$clas description]
 * @param   [type]  $idtog        [$idtog description]
 *
 * @return  [type]                [return description]
 */
    ...
    foreach ($list as $key => $val) {
        ...
        if ($myrow['poster_id'] !== '0') {
            ...
            if ($user or Groupe::autorisation(-127)) {
                ...
                if ($posterdata['femail'] != '') {
                    $useroutils .= '<a class="list-group-item list-group-item-action text-primary text-center text-md-start" href="mailto:' . Spam::anti_spam($posterdata['femail'], 1) . '" target="_blank" title="' . translate("Email") . '" data-bs-toggle="tooltip"><i class="fa fa-at fa-2x align-middle fa-fw"></i><span class="ms-3 d-none d-md-inline">' . translate("Email") . '</span></a>';
                }
        ...
        echo '
                ...
                <div class="card-footer">
                    <div class="row">
                        <div class=" col-sm-6 text-body-secondary small">' . Date::post_convertdate($date_post) . '</div>
...
for ($i = 0; $i < $total_contributeurs; $i++) {
    $contri = User::getUserDataFromId($contributeurs[$i]);
...
if (isset($user)) {
    ...
    if (sql_num_rows($result_LR) == 0) {
        ...
        $sqlR = "UPDATE " . sql_table('forum_read') . " 
...
if ($allow_upload_forum) {
    ...
    $sql2 = "SELECT att_id 
             FROM $upload_table 
             WHERE apli='forum_npds' && topic_id = '$topic' $visible";
...
$sql = "UPDATE " . sql_table('') . "forumtopics SET topic_views = topic_views + 1 WHERE topic_id = '$topic'";
sql_query($sql);
