...
use Npds\Support\Facades\Media;
...
function finishNewUser($uname, $name, $email, $user_avatar, $user_occ, $user_from, $user_intrest, $user_sig, $user_viewemail, $pass, $user_lnl, $C1, $C2, $C3, $C4, $C5, $C6, $C7, $C8, $M1, $M2, $T1, $T2, $B1)
{
    ...
    if (!isset($_SERVER['HTTP_REFERER'])) {
        Log::Ecr_Log('security', 'Ghost form in user.php registration. => NO REFERER', '');
        Spam::L_spambot('', "false");

        include('admin/die.php');
        die();
    } else if ($_SERVER['HTTP_REFERER'] . $NPDS_Key !== $nuke_url . '/user.php' . $NPDS_Key) {
        Log::Ecr_Log('security', 'Ghost form in user.php registration. => ' . $_SERVER["HTTP_REFERER"], '');
        Spam::L_spambot('', "false");
    ...
    if (!$stop) {
        ...
        if ($result) {
            ...
            Mailer::send_email($adminmail, $subject, "Infos :
         Nom : $name
         ID : $uname
...
function userinfo($uname)
{
    ...
    if (stristr($user_avatar, "users_private")) {
        ...
        if (function_exists("theme_image")) {
            if (Theme::image("forum/avatar/blank.gif")) {
    ...
    $posterdata_extend = User::getUserDataExtendFromId($uid);
    ...
    $posterdata = User::getUserDataFromId($uid);
    $useroutils = '';
    ...
    while (list($topic_id, $forum_id, $post_text, $post_time) = sql_fetch_row($result)) {
        ...
        $message = Media::aff_video_yt($message);
...
function mail_password($uname, $code)
{
    ...
    $uname = Hack::removeHack(stripslashes(htmlspecialchars(urldecode($uname), ENT_QUOTES, cur_charset)));
    ...
    if (!$tmp_result) {
        ...
        Log::Ecr_Log('security', 'Lost_password_request : ' . $uname, '');
    }
}
...
function valid_password($code)
{
    ...
    $result = sql_query("SELECT email, pass 
                         FROM " . sql_table('users') . " 
                         WHERE uname='" . Crypt::decrypt($ibid[0]) . "'");
    ...
    if ($email != '') {
        ...
        if ($email == $ibid[0]) {
            ...
            Log::Ecr_Log('security', 'Lost_password_valid NOK Mail not match : ' . $ibid[0], '');
        }
    } else {
        message_pass('<div class="alert alert-danger lead text-center">' . translate("Erreur") . '</div>');
        Log::Ecr_Log('security', 'Lost_password_valid NOK Bad hash : ' . $ibid[0], '');
    }
}
        ...
        if ($email == $ibid[0]) {
            ...
            if ((time() - $ibid[2]) < 86400) {
                ...
                if ($ibid[1] == $passwd) {
                    ...
                    Log::Ecr_Log('security', 'Lost_password_update OK : ' . $uname, '');
                } else {
                    ...
                    Log::Ecr_Log('security', 'Lost_password_update Password not match : ' . $uname, '');
                }
            } else {
                ...
                Log::Ecr_Log('security', 'Lost_password_update NOK Time > 24H00 : ' . $uname, '');
            }
        } else {
            message_pass('<div class="alert alert-danger lead text-center">' . translate("Erreur : Email invalide") . '</div>');
            Log::Ecr_Log('security', 'Lost_password_update NOK Mail not match : ' . $uname, '');
        }
    } else {
        message_pass('<div class="alert alert-danger lead text-center">' . translate("Erreur") . '</div>');
        Log::Ecr_Log('security', 'Lost_password_update NOK Empty Mail or bad user : ' . $uname, '');
    }
}
...
function edituser()
{
    ...
    User::memberMenu($userinfo['mns'], $userinfo['uname']);
        ...
        if ((isset($pass)) && ("$pass" != "$vpass")) {
            ...
            if (!$stop) {
                ...
                if ($B1 != 'none') {
                    ...
                    include_once("modules/upload/Support/clsUpload.php");
                ...
                if ($raz_avatar) {
                    if (strstr($user_avatar, '/storage/users_private')) {
                ...
                if ($pass != '') {
                    ...
                    sql_query("UPDATE " . sql_table('users') . " 
                               SET 
                                    name='$name', 
                                    email='$email', 
                                    femail='" . Hack::removeHack($femail) . "', 
                                    url='" . Hack::removeHack($url) . "', 
                                    pass='$pass', hashkey='1', 
                                    bio='" . Hack::removeHack($bio) . "', 
                                    user_avatar='$user_avatar', 
                                    user_occ='" . Hack::removeHack($user_occ) . "', 
                                    user_from='" . Hack::removeHack($user_from) . "', 
                                    user_intrest='" . Hack::removeHack($user_intrest) . "', 
                                    user_sig='" . Hack::removeHack($user_sig) . "', 
                                    user_viewemail='$a', 
                                    send_email='$u', 
                                    is_visible='$v', 
                                    user_lnl='$w' 
                ...
                if (sql_num_rows($result) == 1) {
                    sql_query("UPDATE " . sql_table('users_extend') . " 
                               SET 
                                    C1='" . Hack::removeHack($C1) . "', 
                                    C2='" . Hack::removeHack($C2) . "', 
                                    C3='" . Hack::removeHack($C3) . "',
                                    C4='" . Hack::removeHack($C4) . "',
                                    C5='" . Hack::removeHack($C5) . "', 
                                    C6='" . Hack::removeHack($C6) . "', 
                                    C7='" . Hack::removeHack($C7) . "', 
                                    C8='" . Hack::removeHack($C8) . "', 
                                    M1='" . Hack::removeHack($M1) . "', 
                                    M2='" . Hack::removeHack($M2) . "', 
                                    T1='" . Hack::removeHack($T1) . "', 
                                    T2='" . Hack::removeHack($T2) . "', 
                                    B1='$B1' 
                               WHERE uid='$uid'");
                } else {
                    $result = sql_query("INSERT 
...
function savejournal($uid, $journal, $datetime)
{
    ...
    if ($uid == $vuid) {
        ...
        $journal = DataImage::dataimagetofileurl($journal, 'storage/users_private/' . $cookie[1] . '/jou'); //
        $journal = Hack::removeHack(stripslashes(Str::FixQuotes($journal)));
