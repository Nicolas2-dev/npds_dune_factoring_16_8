...
use Npds\Contracts\Security\HackInterface;


/**
 * Hack class
 */
class Hack implements HackInterface
{
    /**
    ...
    /**
     * Permet de rechercher et de remplacer "some bad words" dans une chaine
     *
     * @param   [type]  $Xstring  [$Xstring description]
     *
     * @return  [type]            [return description]
     */
    public static function removeHack($Xstring)
    {
        if ($Xstring != '') {
    ...
    function MNSremoveHack($Xstring)
    {
        ...
        if ($tiny_mce) {
            if (!$blog_editor) {
                $tiny_mce_theme = 'full';
                $blog_editor = Editeur::fetch('tiny_mce', 'begin') . Editeur::fetch('story', 'false') . Editeur::fetch('tiny_mce', 'end');
            }
        }
