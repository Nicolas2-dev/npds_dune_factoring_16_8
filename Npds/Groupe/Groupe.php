...
use Npds\Contracts\Groupe\GroupeInterface;
...
class Groupe implements GroupeInterface
{
    ...
    public static function autorisation($auto)
    {
        ...
        if ($auto > 1) {
            $tab_groupe = static::valid_group($user);
    ...
    public static function fab_espace_groupe($gr, $t_gr, $i_gr)
    {
        ...
        if ($rsql['groupe_forum'] == 1) {
            ...
            $content .= '<hr /><div class="">' . $lst_for_tog . '<i class="fa fa-list-alt fa-2x text-body-secondary ms-3 align-middle" title="' . translate("Groupe") . '(' . $gr . '): ' . translate("forum") . '." data-bs-toggle="tooltip" ></i>&nbsp;<a class="text-uppercase" href="forum.php">' . translate("Forum") . '</a>' . $nb_for_gr . $lst_for . '</div>' . "\n";
        }
        ...
        if ($rsql['groupe_pad'] == 1) {
            ...
            $content .= '<hr /><div class="">' . $lst_doc_tog . '<i class="fa fa-edit fa-2x text-body-secondary ms-3 align-middle" title="' . translate("Co-rédaction") . '" data-bs-toggle="tooltip" data-bs-placement="right"></i>&nbsp;<a class="text-uppercase" href="modules.php?ModPath=wspad&ModStart=wspad&member=' . $gr . '" >' . translate("Co-rédaction") . '</a>' . $nb_doc_gr . $lst_doc . '</div>' . "\n";
        }
        //<= wspad
        ...
        if ($rsql['groupe_blocnote'] == 1) {
            ...
            include_once("modules/bloc-notes/Controllers/bloc-notes.php");
     ...
     * @return  [type]         [return description]
     */
    public static function fab_groupes_bloc($user, $im)
    {
        global $user;
        ...
        while (list($groupe_id, $groupe_name, $groupe_description) = sql_fetch_row($result)) {
            ...
            if (!file_exists('users_private/groupe/ask4group_' . $userdata[0] . '_' . $groupe_id . '_.txt') and !static::autorisation($groupe_id)) {
                if (!static::autorisation(-1)){
