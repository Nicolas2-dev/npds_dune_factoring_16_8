...
if ($subscribe)
    if (Mailer::isbadmailuser($userinfo['uid']) === false) { //proto
...
if ($member_invisible) {
    if ($userinfo['is_visible'] == 1) $checked = false;
    else $checked = true;
...
// ---- SIGNATURE
$asig = sql_query("SELECT attachsig FROM " . sql_table('users_status') . " WHERE uid='" . $userinfo['uid'] . "'");
...
// --- EXTENDER
if (file_exists("Npds/Sform/extend-user/extender/formulaire.php"))
    include("Npds/Sform/extend-user/extender/formulaire.php");
// --- EXTENDER
