...
class Metalang 
{
    ...
    {
        return Hack::removeHack(stripslashes(htmlspecialchars(urldecode($arg), ENT_QUOTES, cur_charset)));
    }

    // Cette fonction est utilisée pour intégrer des smilies et comme service pour theme_img()
    public static function MM_img($ibid)
    {
    ...
    public static function charg($funct, $arguments)
    {
        if (is_array($arguments)) {
            array_walk($arguments, [static::class, 'arg_filter']);
    ...
    {
        global $glossaire, $admin, $NPDS_debug, $NPDS_debug_str, $NPDS_debug_cycle;
